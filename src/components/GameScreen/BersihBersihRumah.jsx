...
import kulitpisangImg from "../img/kulitpisang.png";
...
const BersihBersihRumah = ({
  ...
  const [kulitPisangs, setKulitPisangs] = useState([]);
  ...
  useEffect(() => {
    ...
    setPopupInfo({
      visible: true,
      message: `Permainan selesai! Kulit pisang yang dibersihkan: ${score}`,
    });
  ...
  // Function to drop kulit pisang with random delay between drops
  const dropKulitPisang = () => {
    if (isGameOver) return;

    // Drop one kulit pisang
    setKulitPisangs((prev) => [
      ...prev,
  ...
  // Move kulit pisangs down every 100ms (adjusted with speed)
  useEffect(() => {
    if (isGameOver) return;
  ...
  // Catch kulit pisangs when near player
  useEffect(() => {
    setKulitPisangs((prev) =>
      prev.filter((item) => {
    ...
    return (
      ...
      >
        ...
        <p style={{ fontSize: 18, color: "#495057" }}>
          Kulit pisang yang berhasil dibersihkan:{" "}
  ...
  return (
    ...
    >
      ...
      >
        Kulit Pisang Dibersihkan: {score}
      </div>
      ...
      >
        ...
        {/* Kulit pisangs falling */}
        {kulitPisangs.map((item) => (
          <img
            key={item.id}
            src={kulitpisangImg}
            alt="kulit pisang"
...
export default BersihBersihRumah;