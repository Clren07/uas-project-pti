...
import souvenirImg from "../img/souvenir.png";
...
const BelanjaSouvenir = ({
  ...
  useEffect(() => {
    ...
    setPopupInfo({
      visible: true,
      message: `Permainan selesai! Souvenir yang ditangkap: ${score}`,
    });
  ...
  // Function to drop souvenir with random delay between drops
  const dropSouvenir = () => {
    if (isGameOver) return;
  ...
  // Move souvenirs down every 100ms (adjusted with speed)
  useEffect(() => {
    if (isGameOver) return;
  ...
  // Catch souvenirs when near player
  useEffect(() => {
    setSouvenirs((prev) =>
      prev.filter((item) => {
    ...
    return (
      ...
      >
        ...
        <p style={{ fontSize: 18, color: "#495057" }}>
          Souvenir yang berhasil ditangkap:{" "}
...
export default BelanjaSouvenir;
